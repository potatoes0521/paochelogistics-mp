...
class TransportEdit extends Component { 
  ...
  componentWillUnmount() {
    Storage.removeStorage('transport_details')
  }
   ...
   */
  getTransportDetails() {
    Storage.getStorage('transport_details').then(res => {
  ...
  submit() {
    ...
    // }
    let sendData = {
      userId: this.transportInfo.userId,
      mobile,
      merchantName,
    ...
    api.transport.editTransport(sendData, this)
      .then(() => {
        let title = '添加成功'
        if (this.pageParams.pageType === 'edit') { 
          title = '编辑成功'
